...
are keys, and values stored in segments are values.

Flow specification is immutable for all the operators except for source and processing ones. 
These operators cannot change the specification, but rather they can drop or add data tuples 
having that specification. As opposed to that, source operators are useful to create flow 
specification in the early stages of workflow design. Requesters can add data segments and 
give names to these segments to be utilized down the flow. In processing operators, flow 
specification can be redefined similar to source operators.
